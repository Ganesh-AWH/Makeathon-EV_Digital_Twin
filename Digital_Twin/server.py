...
def update_vehicle(vid):
    ...
    # --- Predictive Battery Health (SoH) ---
    base_degrad = 0.001 * dt
    # Temperature stress (exponential above 65C)
    if v["temp"] > 65:
        base_degrad += ((v["temp"] - 65) ** 1.5) * 0.0005 * dt
    # RPM/Load stress
    if v["rpm"] > 4000:
        base_degrad += (v["rpm"] - 4000) * 0.000005 * dt
    # Mode multiplier
    if v["mode"] == "sport":
        base_degrad *= 1.5
        
    v["health"] -= base_degrad
    v["health"] = max(0.0, min(100.0, v["health"]))
